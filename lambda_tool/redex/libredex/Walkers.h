  ...
  class parallel {
   public:
    ...
    static void methods(
        ...
        const WalkerFn& walker,
        size_t num_threads = redex_parallel::default_num_threads()) {
          // the first argument is a lambda function again
    ...
    // Call `walker` on all methods in `classes` in parallel. Then combine the
    // Accumulator objects with Sum. this is like a for loop on methods This
    // version doesn't pass an Accumulator object to the walker -- instead the
    // walker returns a fresh Accumulator object which gets summed up.
    //
    // WalkerFn should accept a `DexMethod*` and return `Accumulator`.
    // [Note for templated function]: type-parameter-key is either typename or class. There is no difference
    ...
    // argument deduction: Classes, WalkerFn
    static Accumulator methods(
        const Classes& classes,
        const WalkerFn& walker,
        size_t num_threads = redex_parallel::default_num_threads(),
        Accumulator init = Accumulator()) {
      auto reduce = Reduce();
      // it calls the previous version of the methods function, by creating a wrapper around the walkerfn
