...
void IRTypeChecker::run() {
  printf("check usage of is_bottom()\n");
  TypeDomain bottom_element(IRType::BOTTOM);
  if (bottom_element.is_bottom()) {
    printf("success bottom check\n");
  } else {
    printf("fail bottom check\n");
  }
  TypeDomain top_element(IRType::TOP);
  if (top_element.is_top()) {
    printf("success top check\n");
  } else {
    printf("fail top check\n");
  }
