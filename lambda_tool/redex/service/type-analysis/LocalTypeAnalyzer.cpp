...
bool RegisterTypeAnalyzer::analyze_default(const IRInstruction* insn,
  ...
  std::cout << "use default semantics" << SHOW(insn) << std::endl;
  if (opcode::is_a_load_param(insn->opcode())) {
    return true;
  }
  std::cout << "to top" << std::endl;
  if (insn->has_dest()) {
...
bool RegisterTypeAnalyzer::analyze_invoke(const IRInstruction* insn,
  ...
  std::cout << "analyze_invoke" << SHOW(insn) << std::endl;
  std::cout << "function type is" << insn->get_method()->get_proto()->get_rtype() << std::endl;
  std::cout << "set return type to"
            << DexTypeDomain(insn->get_method()->get_proto()->get_rtype(), Nullness::NOT_NULL) << std::endl;
  if (insn->has_dest()) {
    env->set(insn->dest(), DexTypeDomain(insn->get_method()->get_proto()->get_rtype(), Nullness::NOT_NULL));
  } else {
    env->set(RESULT_REGISTER, DexTypeDomain(insn->get_method()->get_proto()->get_rtype(), Nullness::NOT_NULL));
  }
  return true;
}
bool RegisterTypeAnalyzer::analyze_const(const IRInstruction* insn,
  ...
  std::cout << "analyze_const" << SHOW(insn) << std::endl;
