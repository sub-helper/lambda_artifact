-- derived from q67, udf in projection and predicate, no improv using cache,withnullcheck;
select
...
order by i_category
        ...
        ,rk
limit 100;
-- derived from q67, udf in projection and predicate, no improv using cache,withoutnullcheck;
select
from (select i_category
            ,i_class
            ,i_brand
      ...
      from (select i_category
       ...
       where  ss_sold_date_sk=d_date_sk
          and ss_item_sk=i_item_sk
          and ss_store_sk = s_store_sk
          and d_month_seq between 1181 and 1181+11 and sudf_string_manipulation(i_item_desc, ',',',') != ''
       group by  rollup(i_category, i_class, i_brand, i_product_name, d_year, d_qoy, d_moy,s_store_id, i_des))dw1) dw2
where rk <= 100
order by i_category
        ,i_class
        ...
        ,rk
limit 100;
-- derived from q65, udf in projection and predicate, no improv using cachewithnullcheck;
SELECT s_store_name, 
...
ORDER  BY s_store_name, 
          i_item_desc
LIMIT 100; 
-- derived from q65, udf in projection and predicate, no improv using cachewithoutnullcheck;
SELECT s_store_name, 
sudf_string_manipulation(i_item_desc, ',',',') i_des,               sc.revenue, 
               i_current_price, 
               i_wholesale_cost, 
...
WHERE  sb.ss_store_sk = sc.ss_store_sk 
       AND sc.revenue <= 0.1 * sb.ave 
       AND s_store_sk = sc.ss_store_sk 
       AND i_item_sk = sc.ss_item_sk AND sudf_string_manipulation(i_item_desc, ',',',') != ''
ORDER  BY s_store_name, 
          i_item_desc
LIMIT 100; 
-- derived from q92, udf in predicate, no improv using cachewithnullcheck;
SELECT 
...
LIMIT 100; 
-- derived from q92, udf in predicate, no improv using cachewithoutnullcheck;
SELECT 
         Sum(ws_ext_discount_amt)
FROM     web_sales , 
         item , 
         date_dim 
WHERE    i_manufact_id = 718 AND sudf_string_manipulation(i_item_desc, ',',',') != ''
AND      i_item_sk = ws_item_sk 
AND      d_date BETWEEN '2002-03-29' AND      ( 
                  Cast('2002-03-29' AS DATE) +  INTERVAL '90' day) 
AND      d_date_sk = ws_sold_date_sk 
