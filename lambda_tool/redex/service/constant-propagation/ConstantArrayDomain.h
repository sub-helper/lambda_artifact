...
class ConstantArrayDomain final
 ...
 public:
  ...
  explicit ConstantArrayDomain(uint32_t length) {
    // access the parameter length by copy
    // len is the return value
    mutate_array_length(
  ...
  // we override this function because we need a canonicalize step
 ...
 private:
  ...
  const ArrayLengthDomain& array_length() const {
    // get the first variable of the base class
  ...
  // the Domain is the array value's domain
  ConstantArrayDomain<Domain>& mutate_array_length(
      std::function<void(ArrayLengthDomain*)> f) {
    // invoke f on the first member variable
    /*
    * Derived from the reducedproduct class: Updates a component of the tuple by applying a user-defined operation.
  ...
  ConstantArrayDomain<Domain>& mutate_array_values(
    ...
    // invoke f on the second member variable
  ...
  void canonicalize() {
    ...
    if (array_length().is_top()) {
      // the set_to_top() function is derived from the directProductAbstractDomain.h
      mutate_array_values(
