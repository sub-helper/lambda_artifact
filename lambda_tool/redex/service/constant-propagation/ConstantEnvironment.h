...
//  * respectively.
// current model of constant value
using ConstantValue =
...
// reg stores instance value
// field env is a mapping between field and constant value
using FieldEnvironment =
    sparta::PatriciaTreeMapAbstractEnvironment<const DexField*, ConstantValue>;
// reg env is a mapping between reg and constant value
 ...
 *****************************************************************************/
// final abstract domain that is going to be fed into the fix point iterator
 ...
 * this deserves a lot attention because we focus on the return state
 */
class ReturnState : public sparta::ReducedProductAbstractDomain<ReturnState,
 ...
 public:
  ...
  template <typename Domain>
  Domain get_value() {
    // the get function is a derived function from DisjointUnionAbstractDomain.h
