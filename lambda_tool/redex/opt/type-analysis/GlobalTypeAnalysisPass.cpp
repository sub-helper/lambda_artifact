...
void trace_analysis_diff(DexMethod* method,
  ...
  TRACE(TYPE, 1, "[trace_analysis_diff] Processing Method %s", SHOW(method));
  // printf("Processing Method %s", method->c_str());
  auto& cfg = code->cfg();
  type_inference::TypeInference type_inference(cfg);
  type_inference.run(method);
  auto lenvs = type_inference.get_type_environments();
  // TRACE(TYPE, 1, "Get Type inference result Method %s", SHOW(lenvs));
