...
std::unique_ptr<GlobalTypeAnalyzer> GlobalTypeAnalysis::analyze(
  ...
  for (size_t i = 0; i < m_max_global_analysis_iteration; ++i) {
    ...
    if (gta->get_whole_program_state().leq(*wps)) {
      std::cout << "Done: this approximation is not better than the previous "
                   "one, we are done."
                << std::endl;
      std::unordered_set<const DexMethod*> method_set;
      for (auto it : scope) {
        for (auto it_ : it->get_dmethods()) {
          method_set.insert(it_);
        }
        for (auto it_ : it->get_vmethods()) {
          method_set.insert(it_);
        }
      }
      ...
      for (auto it : method_set) {
        auto type_result = wps->get_return_type(it);
        Json::Value method_entry;
        method_entry["name"] = show(it);
        method_entry["lattice"] = type_result.get_nullness().element();
        std::string class_name = ((it)->get_class())->c_str();
        ...
        std::cout << "print analysis result" << show(it) << " -> "
                  << type_result.get_nullness().element() << std::endl;
        ...
        if (type_result.is_top()) {
          std::cout << "The return type is TOP!" << std::endl;
        } else {
          std::cout << "The return type is not TOP!" << std::endl;
        }
        std::cout << "print analysis result" << it->get_name()->c_str() << "->";
      ...
      for (auto itr: final_result) {
          ...
          if (strcmp(currentdatetime.c_str(), "") == 0) {
            outputJsonFile = analysisOutputPath + "/" + itr.first + "_" + "nullable.json";
          } else {
            outputJsonFile = analysisOutputPath + "/" + currentdatetime + "/" + itr.first + "_" + "nullable.json";
          }
        ...
        // if (!type::is_object(it->get_proto()->get_rtype())) {
        //   std::cout << "The return type is not object\n";
        //   continue;
        // }
      break;
    }
