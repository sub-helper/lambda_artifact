...
class UpCodeNotionPass : public Pass {
 public:
  struct Stats {
  ...
  UpCodeNotionPass() : Pass("UpCodeNotionPass", Pass::ANALYSIS) {}
  ...
  void bind_config() override {
    ...
    bind("white_list_func", {}, m_white_list_func);

  }

  static void process_code(bool is_static,
                            DexType* declaring_type,
                            DexTypeList* args,
                            IRCode*,
                            bool is_branch_hot_check, DexMethod* method1, const std::unordered_set<std::string> m_white_list_func);

 private:
  bool m_check_if_branch_is_hot;
  std::unordered_set<std::string> m_white_list_func;
